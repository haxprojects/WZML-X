...
def doods(url):
    try:
        response = requests.get(f'https://api.pake.tk/dood?url={url}')
        data = response.json()
        ...
        if response.status_code == 200:
            ...
            return DirectDownloadLinkException(f"Error: HTTP Status Code {response.status_code}")
    except Exception as e:
